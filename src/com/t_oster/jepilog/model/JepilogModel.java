...
public class JepilogModel extends AbstractModel implements Serializable
{
  ...
  public String getStartPosition()
  {
    ...
    {
      if (sp.equals(new Point(w, 0)))
      {
        return "top right";
      }
      else
      {
        if (sp.equals(new Point(0, h)))
        {
          return "bottom left";
        }
        else
        {
          if (sp.equals(new Point(w, h)))
          {
            return "bottom right";
          }
          else
          {
            if (sp.equals(new Point(w / 2, h / 2)))
            {
              return "center";
            }
            else
  ...
  public void setStartPosition(String p)
  {
    if (Util.differ(p, this.getStartPosition()))
    {
      ...
      {
        if (p.equals("top right"))
        {
          this.setStartPoint(w, 0);
        }
        else
        {
          if (p.equals("bottom left"))
          {
            this.setStartPoint(0, h);
          }
          else
          {
            if (p.equals("bottom right"))
            {
